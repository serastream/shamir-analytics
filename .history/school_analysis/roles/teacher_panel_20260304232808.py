        ...
        with tab2:
            ...
            if pivot.shape[1] < 2:
                ...
                for node in G.nodes:
                    ...
                    node_x.append(x)
                    node_y.append(y)
                    avg_val = G.nodes[node]["avg"]
                    node_color.append(avg_val)
                    node_size.append(15 + (avg_val / 4))
                    
                    # Улучшенный Tooltip для учителя
                    comm_id = community_map.get(node, "Без группы")
                    node_text.append(
                        f"<b>{node}</b><br>"
                        f"Средний балл: {avg_val:.0f}%<br>"
                        f"Группа схожести №{comm_id}"
                    )
                ...
                fig.add_trace(go.Scatter(
                    ...
                    text=[n.split(' ')[0] for n in G.nodes], # Только фамилия на графике для чистоты
                    ...
                    marker=dict(
                        ...
                        line=dict(color="white", width=2) # Белая обводка делает узлы четче
                    ),
                ))

                fig.update_layout(
                    title="🧠 Карта образовательных связей (кто учится одинаково)",
                    showlegend=False, 
