    ...
    with tab1:
        ...
        with tab2:
            ...
            if pivot.shape[1] < 2:
                ...
                # --- НЕЙТРАЛЬНЫЙ АНАЛИТИЧЕСКИЙ ВЫВОД ---
                st.markdown("---")
                st.subheader("📉 Анализ учебных паттернов")

                # Собираем данные об ошибках (явные аномалии — это более 6-7 общих ошибок)
                suspicious_pairs = []
                student_errors = {
                ...
                for comm in communities:
                    ...
                    for i in range(len(members)):
                        for j in range(i + 1, len(members)):
                            ...
                            # Фиксируем только очень высокую степень сходства
                            if len(common) >= 7: 
                                suspicious_pairs.append(f"{members[i]} и {members[j]}")

                # 1. Секция аномалий (только если они действительно "кричат")
                if suspicious_pairs:
                    st.info(f"🔍 **Замечена высокая синхронность:** У пар {', '.join(suspicious_pairs)} выявлено более 7 идентичных ошибок. Это редкая математическая аномалия, на которую стоит обратить внимание при проверке работ.")
                else:
                    st.success("✅ **Индивидуальный прогресс:** Явных аномалий или повторяющихся цепочек ошибок в классе не обнаружено. Ученики двигаются по программе самостоятельно.")

                # 2. Секция структуры класса
                isolated = [n for n in G.nodes if G.degree[n] == 0]
                if isolated:
                    st.write(f"**Особенности траекторий:** Ученики {', '.join(isolated)} демонстрируют уникальный профиль усвоения материала, не совпадающий с общими группами.")

                # 3. Краткое резюме одной строкой
                if len(communities) > 1:
                    st.caption(f"Класс распределен на {len(communities)} группы со схожими учебными ритмами. В целом обстановка стабильная.")
