        ...
        with tab2:
            ...
            if pivot.shape[1] < 2:
                ...
                if len(communities) > 0:
                    ...
                    # 2. Исправленный расчет: используем [n] вместо (node=n)
                    isolated = [n for n in G.nodes if G.degree[n] == 0]
                    risk_in_groups = [n for n in G.nodes if G.nodes[n].get('avg', 0) < 45 and G.degree[n] > 0]

                    col_res1, col_res2 = st.columns(2)

                    with col_res1:
                        st.write(f"**🧩 Обнаружено групп:** {len(communities)}")
                        st.write(f"**👥 Синхронное ядро:** {len(biggest_comm)} чел.")
                        if risk_in_groups:
                            st.warning(f"⚠️ **Групповой риск:** {len(risk_in_groups)} чел. совершают идентичные ошибки.")
                    
                    with col_res2:
                        st.write(f"**👤 Одиночные траектории:** {len(isolated)} чел.")
                        if isolated:
                            st.info("У этих учеников уникальный стиль ошибок. Им нужны индивидуальные пояснения.")
